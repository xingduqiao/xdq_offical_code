...
function isloginajax(){
  ...
  if(!$userid || !$groupid || !$username){
    ...
    $user=$empire->fetch1("select userid,email,userfen,money from {$dbtbpre}enewsmember where userid='$userid' and username='".$username."' and rnd='".$rnd."' and checked=1 limit 1");
    if(!$user[userid]){
       $zt="nologin";
    } else {
      $u[userid]=$userid;
