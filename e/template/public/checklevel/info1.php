...
function eCheckLevelInfo_ReturnMsgStr($ckuser,$ecms){
	...
	{
		$msgstr="您还未登陆，<a href='$gotourl'><u><font color=red>点击这里</font></u></a>进行登陆操作；注册请<a href='".$public_r['newsurl']."e/member/register/'><u><font color=red>点击这里</font></u></a>。";
	}
...
function eCheckLevelInfo_ReturnViewLevelSay($infor){
	...
	if(empty($infor['userfen']))//不需要扣点
	{
		...
		else
		{
			if($infor['groupid']>0)
			{
				$ViewLevel="需要 <font color=red>[".$level_r[$infor['groupid']]['groupname']."]</font> 级别以上才能查看。";
			}
			else
			{
