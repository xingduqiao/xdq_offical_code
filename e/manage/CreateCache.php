...
function CreateClassCache($bclassid,$exp,$expjs,$expmodjs,$adminclass,$doall,$mid,$addminfocid,$oldmid,$oldaddminfocid,$userid){
	...
	while($r=$empire->fetch($sql))
	{
		...
		if($r[islast])
		{
			...
			{$menutype="file";}
			$infoclassname='<li><a href="javascript:;" data-url="ListNews.php?&bclassid='.$r[bclassid].'&classid='.$r[classid].'"><i class="icon icon-angle-right"></i> <span>'.$r[classname].'</span></a></li>';
			$onmouseup="";
		}
		else
		{
			...
			$infoclassname='<li><h4><i class="icon icon-file-text-o"></i> <span>'.$r[classname].'</span> <i class="icon icon-double-angle-right"></i></h4>';
		}
		$returnr['listenews'].=$infoclassname;
		//JS颜色
		if($r[islast])
		{
		...
		if(empty($r[islast]))
		{
			...
			if(empty($r['wburl']))
			{
			$returnr['listenews'].='<ol>'.$retr['listenews'].'</ol></li>';
			if($havelevel)
			{
