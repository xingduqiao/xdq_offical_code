...
<title><?=$grpagetitle?>_<?=$public_r[sitename]?></title>
...
?> 
    ...
    <section class="news-detail-bg">
			<div class="news-detail-wrap">
				<h3 class="nd-title"><?=$ecms_gr[title]?></h3>
				<p class="nd-desc"></p>
				<div class="nd-time-share clearfix">
					<div class="nts-time"><span>来源：<a href="<?=$grclassurl?>"><?=$class_r[$ecms_gr[classid]][classname]?></a></span>&nbsp;&nbsp;&nbsp;<span> 发布时间：<?=date('Y-m-d',$ecms_gr[newstime])?></span></div>
...
<span id="_userlogin_c" style=" padding:10px; text-align:center; font-size:15px"><?
if($navinfor[point]){
?>
<div class="buynr" classid="<?=$ecms_gr[classid]?>" xxid="<?=$ecms_gr[id]?>">内容加载中..</div>
<?
} else {
?>
<?=$ecms_gr[sfnr]?>
...
?>
					...
					<a href="<?=$grclassurl?>" class="pn-return-list">返回列表 >></a>
				</div>
			</div>
		</section>
