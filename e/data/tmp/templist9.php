...
?> 
  ...
  <section class="inside-pages-banner">
      <div class="ipb-pic"><img src="[!--news.url--]skin/default/nyban<?=$class_r[$navclassid][classid]?>.jpg" /></div>
            <div class="ipb-words">
...
<?php
$classid=$class_r[$GLOBALS[navclassid]][classid]; //获取当前栏目ID
?>
 <?php
$bqno=0;
$ecms_bq_sql=sys_ReturnEcmsLoopBq("select classid, classname, classpath from `[!db.pre!]enewsclass` where bclassid='$classid' order by `classid` asc ",5,24,0,0,0);
if($ecms_bq_sql){
while($bqr=$empire->fetch($ecms_bq_sql)){
$bqsr=sys_ReturnEcmsLoopStext($bqr);
$bqno++;
